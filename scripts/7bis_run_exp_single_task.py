...
EXP_NAME = "0321-milkyway-singletasks"
DIR_NAME = EXP_NAME
CONCAT = True
FEATURE_FOLDER = "0206_wordembed"
LOOP = 0
SELECT_TASKS = ["milkywayvodka", "milkywayoriginal", "milkywaysynonyms"]
TEST_LOCAL = False
...
if __name__ == "__main__":
    ...
    dataset = get_task_df(keep_milk=True)
    if len(SELECT_TASKS):
        dataset = dataset.query("audio_task in @SELECT_TASKS")
    ...
    if len(params) == 0:
        ...
        np.save(
            save_dir / "params.npy",
            {"FEATURES": FEATURES, "SELECT_TASKS": SELECT_TASKS, **RUN_PARAMS},
        )
